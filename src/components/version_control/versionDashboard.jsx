...
const VersionDashboard = () => {
  ...
  const [selectedDeviceArray, setSelectedDeviceArray] = useState([]);
  ...
  const updateVersionList = (selected, item) => {
    const arr = [...data];

    // const arrCopy = arr.map((ele) => {
    //   return ele.id == item.id ? { ...ele, isChecked: selected } : ele;
    // });
    ...
    if (item == "all") {
      ...
      const all = arr.map((ele) => {
        return { ...ele, isChecked: selected };
      });
      setPayloadData(all);
      setData(all);
      setSelectedDeviceArray(all);
      console.log(all);
    } else {
      ...
      const arr2 = all.filter((ele) => {
        return ele.isChecked;
      });
      setPayloadData(arr2);
      setData(all);
      const selectArray = all.filter((ele) => ele.isChecked);
      console.log(all, selectArray);
      console.log(selectArray, arr2);
      setSelectedDeviceArray(arr2);
    }
    // setSelectedDeviceArray((pre) => {
  ...
  const exportDataList = (exportArray) => {
    ...
    exportArray.forEach((item, index) => {
      let objExport = {
        ...
        },Sim2Provider: ${item.sim2Provider ? item.sim2Provider : "NA"}`,
      };
  ...
  const showData = async (type, exportType) => {
    setLoading(true);
    let payload = {
    ...
    if (res) {
      ...
      if (type) {
        ...
        const a1 = res.data.map((ele) => {
          return { ...ele, isChecked: false };
        });
        // setData(res.data);
        setData([...payloadData, ...a1]);
  ...
  const debouncedResults = useMemo(() => {
    return debouce(globalSearcValueHandler, 500);
  }, []);
                ...
                <Grid item sx={{ marginTop: "20px" }}>
                  ...
                  >
                    <Link
                      ...
                      to={{
                        pathname: "/version_update",
                      }}
                      state={{ devices: selectedDeviceArray }}
                    >
                      Submit
                    </Link>
                  </Button>
                      ...
                      >
                        ...
                        >
                          <Checkbox
                            ...
                            // }
                            onChange={(e) =>
                              updateVersionList(e.target.checked, "all")
                            }
                          ></Checkbox>
                        </TableCell>
                          ...
                          <TableRow key={item.id}>
                            <TableCell>
                              <Checkbox
                                ...
                                onChange={(event) =>
                                  updateVersionList(event.target.checked, item)
                                }
                              />
                            </TableCell>
                            <TableCell>
