...
function SimpleDialog(props) {
  ...
  //const [baseUrl,setBaseUrl]=React.useState("http:/192.168.60.217:3000/viewBoxDetails/")
   const [baseUrl,setBaseUrl]=React.useState(config.frontEndUrl+"viewBoxDetails/")
  ...
  return (
    <div style={{}}>
      <Dialog onClose={onClose} open={open}>
        <DialogTitle>{`Box Number - ${data.boxNo}`}</DialogTitle>
        <div style={{ padding: "5px 15px" }}>{`Quantity :- ${
          data && data.currentQuantity ? data.currentQuantity : "NA"
        }`}</div>
